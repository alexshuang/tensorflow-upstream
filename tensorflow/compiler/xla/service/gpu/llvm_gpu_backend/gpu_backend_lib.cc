...
protected:
  ...
  int hit_count=0;
public:
  static bool Find(const std::string& ir, uint64_t& hash, int gfx, std::vector<uint8>& hsaco);
  static void Add(const std::string& ir, uint64_t hash, int gfx, const std::vector<uint8>& hsaco);
};

static HsacoCache g_hsacoCache;

bool HsacoCache::Find(const std::string& ir, uint64_t& hash, int gfx, std::vector<uint8>& hsaco)
{
...
void HsacoCache::Add(const std::string& ir, uint64_t hash, int gfx, const std::vector<uint8>& hsaco)
{
  std::lock_guard<std::mutex> lg(g_hsacoCache.m_mutex);
